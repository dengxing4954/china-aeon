...
import ConfigPage from '../../home/views/ConfigPage.js';
...
class InvoiceService extends Component {
    ...
    handleEjoural = (item, type) => {
        ...
        if (this.props.rqsj) {
            if (this.props.rqsj.split('-')[2].slice(0, 2) == this.props.syspara.dateHSXP) {
                isyellowPrint = true
            }
        }
    ...
    // 请求支付后结果处理
    afterPayHandle = (isPaySuccessed, res, errMsg, isChangeSuccessed) => {
        let ishaveZL = false;
        let change = '0.00';
        let that = this;
        let doOctozzfkDone = () => {
    ...
    //后厨打印方法
    // handleBackPrintFunc = (data, idSheetNo, saleDate, callback) => {
    //     let _this = this
    //     this.setState({ isPrint: false })
    //     //  f = 1时后厨打印机成功  f = 2为出尾票
    //     const handlePager = (pagerParams, f) => {
    //         let flag = window.PagerSystem(pagerParams)
    //         if (flag == '1') {
    //             if (f == 1) {
    //                 data.shift()
    //                 return _this.handleBackPrintFunc(data, idSheetNo, saleDate, callback)
    //             } else {
    //                 if (data.length === 1) {
    //                     _this.setState({ isPrint: true }, () => {
    ...
    //                 },
    //                 onCancel() {
    //                     return handlePager(pagerParams, f)
    //                 }
    //             });
    //         }
    //     }
    ...
    //                 erpCode: this.props.jygs
    //             }
    //             let pagerParams = { no: backPrintNo, code: [selfId] };
    //             this.props.handleBackPrint(req).then(res => {
    //                 if (res && res.retflag === "0") {
    //                     //  message(window.PagerSystem(this.props.fphm));
    //                     // data.shift()
    //                     // window.UpdatePagerNO()
    //                     // return _this.handleBackPrintFunc(data, idSheetNo)
    //                     return handlePager(pagerParams, 1)
    ...
    //                         cancelText: '重試',
    //                         onOk() {
    //                             // if(handlePager(backPrintNo)){
    //                             //     window.Print(_this.tailPrint(data[0], saleDate))
    //                             // }
    //                             return handlePager(pagerParams, 2)
    //                         },
    //                         onCancel() {
    //                             return _this.handleBackPrintFunc(data, idSheetNo, saleDate, callback)
    //                         }
    ...
    //八达通增值写卡
    octocardRecharge = (_total, _isOddChange, _OddChangeState, reHandle, octoRwType, octoRwCardId, octoPollTimes) => {
        let that = this;
    ...
    //八达通已付款未写卡提交拦截
    octozzUnDoneFilter = () => {
        if (!!this.state.query && !!this.state.query && !!this.state.query.djlb && this.state.query.djlb === 'Y3' && (this.state.octozzfkDone === true && this.state.octozzDone === false)) {
    ...
    //更新收银机状态
    updateSyjstate = (syjcurinvbs, syjcurcashje, syjcurinvje) => {
        const req = {
            ...
            syjcurnum: this.props.fphm,//收银机当前小票号
        ...
        return Fetch(
            ...
            if ("0" === res.retflag) {
                let stripCode = moment(new Date()).format('YYMMDD') + this.props.syjh + this.props.fphm;
                localStorage.setItem('fphm', stripCode);
    ...
    //打印回调方法
    printAfter = (printData, isACS, cashTotal, data, isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef) => {
        ...
        if (!isACS && !isSDYN && !isPTFQ && printData.head[0].printtype == "0" && !octoZlzzNeedsPrint) {
            if (this.state.type == "4" || this.state.type == "2" || this.state.type == "Y2") {
                ...
                this.updateSyjstate(a.dealNumbers, a.cash, a.mediaTotal).then((res) => {
                ...
                const updateSyjstateFunc = () => {
                    this.updateSyjstate(a.dealNumbers, a.cash, a.mediaTotal).then((status) => {
                        this.props.init();
                        this.props.update();
                        ...
                        this.setState({ isPrint: false });
                        this.props.history.push(this.props.isDiningHall ? "/square" : "/presale");
                    })
                }
        ...
        if (printData.head[0].printtype === "0" && octoZlzzNeedsPrint === true) {
            ...
            setTimeout(() => {
                window.Print(octozzSalePrintData, (data) => {
                    ...
                    this.printAfter(octozzSalePrintData, isACS, cashTotal, data, isSDYN, isPTFQ, false)
                });
            }, 1000);
            return;
        }
        ...
        if (data && data.code == "0" && printData.head[0].printtype === "1" && !isACS && !isSDYN && !isPTFQ) {
            ...
            if (this.state.type !== "2") {
                ...
                window.Print(newPrintData, (data) => {
                    this.printAfter(newPrintData, isACS, cashTotal, data, isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
                });
                ...
                window.Print(sPrintData, (data) => {
                    this.printAfter(sPrintData, isACS, cashTotal, data, isSDYN)
                });
            }
                ...
                setTimeout(() => {
                    ...
                    Modal.success({
                        ...
                        onOk() {
                            _this.setState({ isPrint: true }, () => {
                                window.Print(newPrintData, (data) => {
                                    _this.printAfter(newPrintData, isACS, cashTotal, data, isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
                                });
                            })
                        }
                    })
    ...
    // mergeGoods = (oldData) => {
    //     let before = [], copyData;
    //     copyData = this.deepCopy(oldData);
    //     for (let key in copyData) {
    //         before.push(copyData[key]);
    //     }
    //     before = before.filter(item => item.goodsType !== "99" && item.goodsType !== "98");//过滤券商品
    //
    ...
    //         let num = 0, total = 0, ysje = 0, zkInfo, pop_details = [], protot;
    //         protot = {...goods[0]};
    ...
    //销售打印
    submitPrint = (hykh, isyellowPrint, ArtcodeMoney, ArtcodeMoneyTotal, DirectMoney, hjzsl, bankTotal, notBankTotal, isTailTotal, isnotTailTotal, isYhrq, goods, pay, cashTotal, isSDYN, printObj, octoZlzzNeedsPrint, isACS, isPTFQ, isPress, isSCB, isMahatan, octozzSalePrintData, octoZlzzOrgiRef) => {
        pay.forEach(item => {
        ...
        if (this.state.octoDjlb == "Y3") {//八达通增值小票
                ...
                onOk() {
                    _this.setState({ isPrint: true }, () => {
                        window.Print(FQPrintData, (data) => {
                            _this.printAfter(FQPrintData, isACS, cashTotal, data, isSDYN, false)
                        });
                    })
                }
            })
                ...
                onOk() {
                    _this.setState({ isPrint: true }, () => {
                        window.Print(ACSFQPrintData, (data) => {
                            _this.printAfter(ACSFQPrintData, isACS, cashTotal, data, isSDYN)
                        });
                    })
                }
            })
                ...
                onOk() {
                    _this.setState({ isPrint: true }, () => {
                        window.Print(CJPrintData, (data) => {
                            _this.printAfter(CJPrintData, isACS, cashTotal, data, isSDYN, isPTFQ)
                        });
                    })
                }
            })
            ...
            Modal.success({
                ...
                onOk() {
                    _this.setState({ isPrint: true }, () => {
                        window.Print(CJPrintData, (data) => {
                            _this.printAfter(CJPrintData, isACS, cashTotal, data, false, isPTFQ)
                        });
                    })
                }
            })
                        ...
                        _this.setState({ isPrint: true }, () => {
                            setTimeout(() => {
                                window.Print(DCPrintData, (data) => {
                                    _this.printAfter(DCPrintData, isACS, cashTotal, data, undefined, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
                                });
                            }, 1000);
                        })
                    }
                    ...
                    _this.setState({ isPrint: true }, () => {
                        setTimeout(() => {
                            window.Print(DeliveryPrintData, (data) => {
                                _this.printAfter(DeliveryPrintData, isACS, cashTotal, data, isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
                            });
                        }, 1000);
                    })
                }
                    ...
                    _this.setState({ isPrint: true }, () => {
                        setTimeout(() => {
                            window.Print(AJPrintData, (data) => {
                                _this.printAfter(AJPrintData, isACS, cashTotal, data, isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
                            });
                        }, 1000);
                    });
                }
            ...
            if ('N' === this.props.syjmain.isprint)
                this.printAfter(salePrintData, isACS, cashTotal, "", isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
            else {
                this.setState({ isPrint: true }, () => {
                    window.Print(salePrintData, (data) => {
                        _this.printAfter(salePrintData, isACS, cashTotal, data, isSDYN, isPTFQ, octoZlzzNeedsPrint, octozzSalePrintData, octoZlzzOrgiRef)
                    });
                })
            }
        }
    ...
    finalSubmit = () => {
        ...
        let isyellowPrint = 'N', ArtcodeMoney = [], ArtcodeMoneyTotal = 0, DirectMoney = 0, isflPrint = false, payACS,
            ...
            notBankTotal = 0.00, cashTotal = 0.00, isYhrq = 'N';
                    ...
                    if (cancelFinalFlag) {
                        ...
                        this.setState({ isPrint: true }, () => {
                            window.Print(cancelFinalPrintData, data => {
                                if (!!data) {
                                    that.printAfter(cancelFinalPrintData, false, cashTotal, data, false);
                                        ...
                                        if (!!flag) {
                                            getStallConfig(XDTHPrintData.goods).then(res => {
                                                if (res) {
                                                    ...
                                                    let list = groupByGoodsList.filter(v => v[0].stallCode !== '');
                                                    let callback = () => {
                                                        that.printAfter(XDTHPrintData, false, cashTotal, "", false)
                                                    }
                                                ...
                                                if (that.props.isDiningHall && !that.props.breadFlag) {
                                                    getStallConfig(XDTHPrintData.goods).then(res => {
                                                        if (res) {
                                                            ...
                                                            let list = groupByGoodsList.filter(v => v[0].stallCode !== '');
                                                            let callback = () => {
                                                                that.printAfter(XDTHPrintData, false, cashTotal, data, false)
                                                            }
                                                            that.handleXDPrintFunc(list, that.props.xph.substring(6), XDTHPrintData.head[0].saleDate, callback, backPrintStallInfo)
            ...
            this.props.deusubmit(params).then(res => {
                if (res) {
                    ...
                    if (sysTime) {
                        let a = sysTime.split(" ");
                        JyTime = a[0].split("-")[2] + "/" + a[0].split("-")[1] + "/" + a[0].split("-")[0] + " " + a[1];
                        if (sysTime.split('-')[2].slice(0, 2) == this.props.syspara.dateHSXP) {
                            isyellowPrint = 'Y'
                        }
                    }
                    let cashTotal = 0;
                    ...
                    if (octoZlzzNeedsPrint === true) {
                        ...
                        let printTemplate = {
                            ...
                            head: [{
                                ...
                                ysje: this.props.zdyftotal,//应收金额
                                refno: this.props.syjh + this.props.fphm,  //ref值
                                fphm: this.props.fphm,  //ref值
                                orgiRefno: this.props.syjh + '' + (parseInt(this.props.fphm) === 9999 ? '0001' : fphmPlus),
                                printtype: "0", //0代表热敏打印，1代表平推
                                printnum: 0,//重打字显示
                                printtime: 1,//打印次数
                                zl: this.state.change, //找零金额
                        ...
                        onOk() {
                            _this.setState({ isPrint: true }, () => {
                                window.Print(DeuPrintData, (data) => {
                                    _this.printAfter(DeuPrintData, isACS, cashTotal, data, isSDYN)
                                });
                            })
                        }
                    })
                                    ...
                                    Modal.success({
                                        ...
                                        onOk() {
                                            that.submitPrint(hykh, isyellowPrint, ArtcodeMoney, ArtcodeMoneyTotal, DirectMoney, hjzsl, bankTotal, notBankTotal, isTailTotal, isnotTailTotal,
                                                isYhrq, goods, pay, cashTotal, isSDYN, printObj, octoZlzzNeedsPrint, isACS, isPTFQ, isPress, isSCB, isMahatan, octozzSalePrintData, octoZlzzOrgiRef);
                                        }
                                    })
                                }
                            } else {
    ...
    componentDidMount() {
        ...
        if (this.props.limitedPays) {//除外付款方式
            ...
            payModeInfo.map((item, index) => {
                if (item.code == this.props.exceptPaycode) {
                    this.setState({
                        paymode: paymode.splice(index, 1)
                    }, () => {
                        noSalesMemoPaymode = [...this.state.paymode];
                        SalesMemoPaymode = [...this.state.paymode];
                        this.state.paymode.map((item, index) => {
            ...
            let paymode = [...payModeInfo]
            payModeInfo.map((item, index) => {
                this.props.exceptPaycodes.map((limit) => {
                    if (item.code == limit.paycode) {
                        finalpaymode.push(paymode.slice(index, index + 1)[0])
                        return;
                    }
                })
            })
        ...
        } else if (!!this.state.query && !!this.state.query && !!this.state.query.djlb && this.state.query.djlb === 'Y3') {
            let paymode = []
            payModeInfo.map((item, index) => {
    ...
    sliceDcDate = (dcData) => {
        ...
        let obj = Object.assign(_dcData, { date: sliceDate });
        return obj
    }

    render() {
        const { goodsList, zdsjtotal, zdyftotal, zddsctotal, vip_name, vip_no, fphm, payModeInfo, flow_no, mkt, syjh, operators, uidlist, syspara, switchEng, sysparaData, popInfo, exceptPaycodes, erpCode } = this.props;
        return (
            <div className="invoice">
                ...
                />
                <InvoiceRight yftotal={this.state.yftotal}
                    zdyftotal={zdyftotal}
                    goodsList={goodsList}
                    addDjlb={this.props.addDjlb}
                    sftotal={this.state.sftotal}
                    onRef={(ref) => { this.child = ref }}
                    change={this.state.change}
                    changename={this.state.changename}
                    overage={this.state.overage}
                    paylist={this.state.payList}
                    payModeInfo={this.state.paymode}
                    IniPaymode={payModeInfo}
                    octoDjlb={this.state.octoDjlb}
                    octozzfkDone={this.state.octozzfkDone}
                    octozzDone={this.state.octozzDone}
                    submitFilter={this.octozzUnDoneFilter}
                    cancelFilter={this.octozzDoneFilter}
                    octoddRecord={this.octoddRecord}
                    submit={this.finalSubmit}
                    cancel={() => this.cancelpay(operators, flow_no, mkt, syjh)}
                    deletepay={(guid, isAmcPay, item) => this.deletepay(operators, flow_no, guid, mkt, syjh, isAmcPay, item)}
                    ...
                    exceptPaycodes={exceptPaycodes}
                    isrenderBox={this.state.isrenderBox}
                    expressNumber={this.props.expressNumber}
                    vip_no={this.props.vip_no}
                    exceptPayData={this.props.exceptPayData}
                    online={this.props.online}
                    cacelButtonDisabled={this.state.cacelButtonDisabled}
                />
                {
                    this.state.isPrint ?
