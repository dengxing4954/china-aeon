...
import CashBox from '../../../views/invoice/views/CashBox';
import BankBox from '../../../views/invoice/views/BankBox';
import ACSBox from '../../../views/invoice/views/ACSBox';
import StockBox from '../../../views/invoice/views/StockBox';
import ValuecardBox from '../../../views/invoice/views/ValuecardBox';
import AllVPayBox from '../../../views/invoice/views/AllVPayBox';
import ImpowerBox from '../../../views/invoice/views/ImpowerBox';
...
import bankConst from '../../../config/bankConst';

let loadDom;
let isFetching = false;
let payACS, payImpowerCode, paySCB, payMahatan, payMZcoupons, payDircoupons, payWZF;

export default class ShowPaybox extends Component {
    ...
    doPayment = (cash, payModeData, extra, bbje, flag) => {
        ...
        const req = {
            ...
            trace: payModeData.code !== paySCB && payModeData.code !== payMahatan ? this.state.traceNumber : '',//银联流水号
            ...
            terminalid: this.state.octopusDeviceId, //终端号
            payno: this.state.octopusCardno ? this.state.octopusCardno : '', //八达通卡号
        ...
        if (payModeData.virtualPayType === 3 && payModeData.cardPayType === "0" || payModeData.cardPayType === "1") {
            req.needHidePayNo = "Y"
        } else if (payModeData.code == paySCB) {
            req.needHidePayNo = "A"
        }
        console.log(req)
        ...
        return Fetch(
            ...
            isFetching = false;
            if (res.retflag === "0") {
                if ((payModeData.code == paySCB || payModeData.code == payMahatan) && this.props.extra.expressNumber !== this.state.traceNumber) {
                    this.SalesMeomoAdd(extra, this.state.traceNumber).then(() => {
            ...
            this.state.payid = '',//付款卡号（储值卡卡号或微信交易单号）
                this.state.traceNumber = '',//银联流水号
                this.state.merchantid = '',//机构号
                this.state.realMerchantid = '',//机构号
                this.state.exDate = '',//有效时间
                this.state.referenceNumber = ''//银行卡交易参考号
            this.state.octopusDeviceId = '',//终端号
                this.state.octopusCardno = '' //八达通卡号
        })
    }
    //银行卡交易接口
    PaymentBankFunc = (total, payModeData, extra, isonline, approvelCode, cardId, yxDate, callback) => {
        //isonline  0是online, 1是offline
        //启动加载框
        ...
        if ((null != payModeData.cardPayType && "2" !== extra.type) && ("0" === payModeData.cardPayType || "5" === payModeData.cardPayType
            ...
            console.log(req);
            // else
            //     req = {command: payModeData.cardPayType + mytime + rand + money + tipsMoney}

            reqData = window["PaymentBank"](JSON.stringify(req));
            // reqData = window["paymentBank"]({type:'02P', receiptNum:extra.flow_no, money:total*10});
            console.log(reqData);
        }
    ...
    constructMethod = (index) => {
        ...
        if (obj.virtualPayType === 0 || mode.code == this.props.syspara.wkzfPaycode) {
            ...
            //     this.payACS(obj.payno, obj.total, obj.installmentTerms, obj.deliveryMemoNumber, obj.expiryDate, obj.posEntryMode, obj.trackData, mode, this.props.extra).then(() => {
            //         resolve(this.SalesMeomoAdd(this.props.extra, obj.deliveryMemoNumber))
            //     })
            // })
            ...
            let _tradeNo = this.props.extra.mkt + "0" + this.props.extra.mkt + moment().format('YYYYMMDDHHmmss') + this.props.extra.fphm;  // 支付单号: erpcode+门店号+收银机号+日期时间
            const req = {
    ...
    payPoints = (total, jfxfBl, consumers_id, payModeData, extra) => {
        ...
        /*
        const req = {
            command_id: "JFSALEEXPENDCERTIFY",
            operators: extra.operators,     //操作员号
            mkt: extra.mkt,         //门店号
            syjh: extra.syjh,       //终端号收银机号
            flow_no: extra.flow_no, //当前流水号
            ...
            sellpayment: [{         //付款方式
                payname: payModeData.name,  //付款名称
                coptype: this.state.points.coupon_type,     //付款劵种
                coupon_is_cash: this.state.points.coupon_is_cash,       //现金券标记(填入支付行)
                coupon_event_scd: this.state.points.coupon_event_scd,   //用券档期
                coupon_event_id: '5',       //用券活动id
                coupon_policy_id: this.state.points.coupon_policy_id,   //用券策略id
                payno: '625947412525563',   //付款卡号
                paycode: payModeData.paycode,
                hl: payModeData.hl,         //汇率
                consumers_id,               //会员id
                coupon_mutex_arr: this.state.points.coupon_mutex,       //券互斥规则
                coupon_group: extra.coupon_group,           //券账户分组
                kye: extra.balance,         //余额
                ybje: "",            //原币金额
                total: extra.total,  //付款金额
            }],
            scene: "0"//0-普通 1-除外 3- 扣回
        };
        return Fetch(
            {
                url: Url.base_url,
            ...
            if ("0" === res.retflag) {
                ...
                //返回支付信息
                this.searchPoints(consumers_id);//再次查询
            } else {
                throw new Error("积分消费失败" + res.retflag + "----" + res.retmsg);
            }
        }).catch((error) => {
            console.error('error', error);
            throw new Error(error);
    ...
    render() {
        const { payModeData, extra, yfzje, sjzje, onAfterPay, onHidePay, sftotal, syspara, afterZKpay } = this.props;
        ...
        if (payModeData.virtualPayType == '3' && payModeData.code !== payACS && payModeData.code !== payImpowerCode && payModeData.code !== paySCB && payModeData.code !== payMahatan) {
        ...
        } else if (payModeData.code === payACS && payACS !== null && payACS !== undefined) {
            return (
                <ACSBox
                    syyf={sftotal}
                    hidePayDialog={onHidePay}
        ...
        } else if ((payModeData.code === payImpowerCode && payImpowerCode !== null && payImpowerCode !== undefined) || (payModeData.code === paySCB && paySCB !== null && paySCB !== undefined) || (payModeData.code === payMahatan && payMahatan !== null && payMahatan !== undefined)) {
            return (
                <ImpowerBox
        ...
        } else if (payModeData.virtualPayType == '4' || payModeData.virtualPayType == '2' || payModeData.paytype === '3') {
        ...
        } else if (payModeData.virtualPayType == '0' || payModeData.code == syspara.wkzfPaycode) {
        ...
        } else if (payModeData.virtualPayType == '5' || payModeData.paytype === '9' || payModeData.name === '融合支付') {
